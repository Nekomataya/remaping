...
//	nas.GUI.setTabPanel(nas.Pref,["一般環境","メディア設定","動作設定","作画設定"],0,0,9,19);
    if (true) {
        nas.GUI.setTabPanel(nas.Pref, [nas.localize(nas.uiMsg["Common"]),
...
//画面アップデート
    nas.Pref.tabPanel[1].MediaList.update = function () {
        ...
        if (this.list.selected != nas.inputMedias.selected) {
            this.list.check(nas.inputMedias.selected);
        }
...
//画面アップデート
    nas.Pref.tabPanel[1].oMediaList.update = function () {
        ...
        if (this.list.selected != nas.outputMedias.selected) {
            this.list.check(nas.outputMedias.selected)
        }
...
//=========================================================タブパネル２/詳細動作設定
    ...
    nas.GUI.addStaticText(nas.Pref.tabPanel[2], nas.localize({
        en: "footage filter(RegExp)",
        ja: "読み込みフィルタ(正規表現)"
    }), 0, 0, 8, 1).justify = "right";//"読み込みフィルタ"
...
//Tab[0]
//nas_common
    nas.Pref.tabPanel[0].Uname.onChange = function () {
        nas.CURRENTUSER = this.text
    };
    nas.Pref.tabPanel[0].bResolution.onChange = function () {
        nas.RESOLUTION = this.text / 2.54;
        this.text = nas.Dpi()
    };
    ...
    nas.Pref.tabPanel[0].stLength.onChange = function () {
        ...
        if (!isNaN(frames)) {
            nas.SheetLength = frames
        }
        ;
        this.text = nas.Frm2FCT(nas.SheetLength, 3);
    };
...
//TitleDB
    ...
    nas.Pref.tabPanel[0].TitleSelector.delEntry.onClick = function () {
        if (this.parent.list.options.length == 1) {
            alert("エントリが0になるので削除できません。");
            return;
        }
    ...
    nas.Pref.tabPanel[0].TitleSelector.myTitle.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[0].TitleSelector.myPrefix.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[0].TitleSelector.myCode.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[0].TitleSelector.linkIM.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[0].TitleSelector.linkOM.onChange = function () {
        this.parent.chgvalue("Name")
    };
...
//IM DB
    ...
    nas.Pref.tabPanel[1].MediaList.imName.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].MediaList.imWidth.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].MediaList.imFrameRatio.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].MediaList.imResolution.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].MediaList.imFrameRate.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].MediaList.imPegID.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].MediaList.imPegX.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].MediaList.imPegY.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].MediaList.imPegR.onChange = function () {
        this.parent.chgvalue("Name")
    };
...
//OM DB
    ...
    nas.Pref.tabPanel[1].oMediaList.omName.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].oMediaList.omWidth.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].oMediaList.omHeight.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].oMediaList.omPa.onChange = function () {
        this.parent.chgvalue("Name")
    };
    nas.Pref.tabPanel[1].oMediaList.omFramerate.onChange = function () {
        this.parent.chgvalue("Name")
    };
...
//Tab[2]
    nas.Pref.tabPanel[2].impFilt.onChange = function () {
        nas.importFilter = (this.text.match(/^\/.*\/[rgi]$/)) ? eval(this.text) : new RegExp(this.text)
    };
    nas.Pref.tabPanel[2].cellFilt.onChange = function () {
        nas.cellRegex = (this.text.match(/^\/.*\/[rgi]$/)) ? eval(this.text) : new RegExp(this.text)
    };
    nas.Pref.tabPanel[2].bgFilt.onChange = function () {
        nas.bgRegex = (this.text.match(/^\/.*\/[rgi]$/)) ? eval(this.text) : new RegExp(this.text)
    };
    nas.Pref.tabPanel[2].mgFilt.onChange = function () {
        nas.mgRegex = (this.text.match(/^\/.*\/[rgi]$/)) ? eval(this.text) : new RegExp(this.text)
    };
    nas.Pref.tabPanel[2].loFilt.onChange = function () {
        nas.loRegex = (this.text.match(/^\/.*\/[rgi]$/)) ? eval(this.text) : new RegExp(this.text)
    };
    /*
...
//tab[3]
    ...
    nas.Pref.tabPanel[3].nlOpc.onClick = function () {
        nas.axe.newLayerTpr = this.value
    };
    nas.Pref.tabPanel[3].nlOpcValue.onChange = function () {
        var myNumber = new Number(this.text);
        if ((myNumber <= 100) && (myNumber >= 0)) {
            nas.axe.onsOpc = myNumber / 100
        } else {
            this.text = (nas.axe.onsOpc * 100);
        }
    }
    nas.Pref.tabPanel[3].chgColor = function () {
        for (var ix = 0; ix < nas.axe.lyBgColors.length; ix++) {
            if (nas.Pref.tabPanel[3]["rb" + ix].value) {
                nas.axe.lyBgColor = ix;
                break;
            }
        }
        for (var ix = 0; ix < nas.axe.ovlBgColors.length; ix++) {
            if (nas.Pref.tabPanel[3]["rbo" + ix].value) {
                nas.axe.ovlBgColor = ix;
                break;
            }
        }
    };
    ...
    nas.Pref.tabPanel[3].focusInterlocking.onClick = function () {
        nas.axe.focusMove = this.value;
    }
    if (nas.Version.psAxe >= "1.1.0") {
        nas.Pref.tabPanel[3].playheadMoveToOptKey.onClick = function () {
            nas.axe.useOptKey = this.value;
        }
        nas.Pref.tabPanel[3].playheadSkipFrames.onChange = function () {
            var myNumber = new Number(this.text);
            if ((myNumber <= 60) && (myNumber >= 0)) {
                nas.axe.skipFrames = myNumber;
            } else {
                this.text = nas.axe.skipFrames;
            }
        }
    }
    nas.Pref.tabPanel[3].selectDialog.onClick = function () {
        nas.axe.dmDialog = this.value;
    }
    nas.Pref.tabPanel[3].pegBlendMode.onClick = function () {
        nas.axe.pegBlend = this.value;
    }
    nas.Pref.tabPanel[3].frameOpacity.onClick = function () {
        nas.axe.frameOpc = this.value;
    }
//ショートカットの設定　（ファイルコピー）
//shortcutToolsSetup
...
//ターゲットフォルダが無ければ作る
            ...
            if (!myTargetFolder.exists) {
                if (myTargetFolder.create()) {
                    ...
                    alert(nas.localize({
                        en: "It failed to create a folder. It must be executed by a user with administrative privileges",
...
//削除　nasフォルダは残置?
    ...
    nas.Pref.tabPanel[3].utInstall.onClick = function () {
        setShortcut("install");
    };
    nas.Pref.tabPanel[3].utRemove.onClick = function () {
        setShortcut("remove");
    };
//ショートカットの設定は、選択時に実行
