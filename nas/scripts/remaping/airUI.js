...
function selectCommand(event) {
	...
	break;
	case "dropdownMenuBar":sWitchPanel("memu");
	break;
	...
	break;
	case "memo":sWitchPanel("memoArea");
	break;
	case "sheetHeader":sWitchPanel("SheetHdr");
	break;
	case "toolBar":sWitchPanel("ToolBr");
	break;
...
//		var fName=encodeURI([XPS.scene.toString(),XPS.cut.toString()].join("_")+'\.xps');
//		var fName=encodeURI([XPS.title,XPS.scene.toString(),XPS.cut.toString()].join("c")+'\.xps');
		var fName=encodeURI(xUI.getFileName()+'\.xps');
...
/* ダメパターン　正常終了時には正しいデータ形式だが　キャンセル時に存在しないプロパティを決め込みで記述してある
	var myTarget =_Adobe.JSXInterface.call("eval","(new File('"+fileChooser+"').saveDlg('SaveAs')).fullName;");
