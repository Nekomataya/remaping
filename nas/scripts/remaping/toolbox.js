...
case	13	:		//Enter 標準/次スピン・シフト/前スピン・コントロール/interpSpin
	if(chkCtrl(e)){interpSign();return false;}
	if (xUI.edchg){
...
case	40	:		//[shift]+時はセレクションの調整 [ctrl]+時はさらにスピン量の調整も兼ねる
 	if (	chkShift(e) &&
		...
		if((chkCtrl(e))||(xUI.spinSelect)) xUI.spin("update");
	}else{
		//通常入力処理
	 if(! chkCtrl(e)){
...
case	83 :	alert("SSS");	//[ctrl]+[S]/ Save or Store document
	if (chkCtrl(e)) {
		 if(chkShift(e)){xUI.storeDocument("as");}else{xUI.storeDocument();}
		return false;
	}else{
		return true
	}
...
function dialogSpin(param)
{
	...
	if(doSpin) 
	{
		...
		XPS.xpsTracks[xUI.Select[0]][xUI.Select[1]]=entry;//直更新
		document.getElementById(xUI.Select[0]+"_"+xUI.Select[1]).innerHTML=entry;//
	}
		document.getElementById("iNputbOx").value="";//入力エリア消去
	return false;
}
*/
//サブミットの際の動作
...
//
case	"spin_V"	:
	xUI.spin(document.getElementById(id).value);break;
case	"v_up"	:	;//スピン関連
...
//	xUI.put(xUI.favoriteWords[document.getElementById(id).selectedIndex]);
	xUI.put(EXword);
	xUI.spin("fwd");
...
//Selector+=(selected==0)?'<option selected/>台詞':'<option />台詞';
...
//Selector+=(selected==(xUI.SheetWidth-1))?'<option selected/>MEMO.':'<option />MEMO.';
...
//入力ボックスにイベントハンドラを設定
//	document.getElementById("iNputbOx").onkeydown=xUI.keyDown;
//	document.getElementById("iNputbOx").onkeypress=xUI.keyPress;
//	document.getElementById("iNputbOx").onkeyup=xUI.keyUp;
}
