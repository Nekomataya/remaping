...
//指定範囲をハイライト
	for (C=range[0][0];C<=range[1][0];C++){
		for (L=range[0][1];L<=range[1][1];L++){
			if((C<0) || (L<0)||(C>=XPS.xpsBody.length)||(L>=XPS.xpsBody[C].length)){
...
//alert(fkPos)
	if ((fkPos[0]<0)||(fkPos[1]<0)||(fkPos[0]>XPS.xpsBody.length)||(fkPos[1]>XPS.xpsBody[0].length)){return false}
...
形式:	[kEyword,vAlue]
	...
	レイヤーコレクションの内包プロパティは　"name.1.layers"等の"."接続した倒置アドレスで指定
...
Xps.Layerオブジェクトのプロパティ
	name	;//識別用タイムラインラベル　編集対象
...
//		UNDO[2]=[入力ターゲット複製,現在値];
		UNDO[2]=[datastream[0],XPS.layers[myTarget[1]][myTarget[0]]];
		XPS.layers[myTarget[1]][myTarget[0]]=datastream[1];//入力値を設定
	}else{
//単独プロパティ変更
...
xUI.syncSheetCell=function(startAddress,endAddress){
	if((! startAddress)||(! endAddress)){
		startAddress=[0,0];
		endAddress  =[XPS.xpsBody.length,XPS.xpsBody[0].length];
	}
...
if((r>=0)&&(r<XPS.xpsBody.length)&&(f>=0)&&(f<XPS.xpsBody[r].length)){
...
break;
case	"mousedown":
	if(this.edmode==0)xUI.changeColumn(tln+1,2*pgn+cbn);
break;
}
return 		;
	}
