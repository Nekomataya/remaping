...
Map.dataChack(グループラベル,XPSトークン)
Map.dataChack(XPSトークン)


マップのメソッドで実装
与えられたトークンを検索してマップ内のエントリを返す。
戻り値
	Map.groups[idx].entrys[idx]	マップエントリオブジェクト
		または
	null
...
グループオブジェクトに Map.groups[idx].getByName(トークン) を実装して
そのラッパとして動作させるのが良さそう。

Map.getGroupByName("グループ名")
